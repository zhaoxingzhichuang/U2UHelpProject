...
import com.megagao.production.ssm.domain.authority.SysUserOption;
...
import com.megagao.production.ssm.domain.vo.UserVO;
...
public class UserServiceImpl implements UserService {
	...
	@Override
	public List<SysUserOption> getList() throws Exception{
		
		//分页处理
		List<UserVO> list = sysUserMapper.findUserVo();
		List<SysUserOption> list2 = new ArrayList<SysUserOption>(list.size());
		for(int i = 0; i<list.size(); i++) {
			list2.add(new SysUserOption(list.get(i).getId(),list.get(i).getUsername()));
		}
		return list2;
	}
