...
<table class="easyui-datagrid" id="employeeList" title="会员列表" 
    ...
    <thead>
        <tr>
        	...
        	<th data-options="field:'empId',align:'center',width:100">会员编号</th>
            <th data-options="field:'empName',align:'center',width:100">会员姓名</th>
            ...
            <th data-options="field:'education',align:'center',width:100">QQ</th>
            <th data-options="field:'educationForm',align:'center',width:100">手机号</th>
...
<div  id="toolbar_employee" style=" height: 22px; padding: 3px 11px; background: #fafafa;">  
    ...
    <div id="search_employee" style="float: right;">
        ...
        <div id="menu_employee" style="width:120px"> 
			<div data-options="name:'employeeId'">会员编号</div> 
			<div data-options="name:'employeeName'">会员名称</div>
		</div>     
    </div>  

</div>  

<div id="employeeEditWindow" class="easyui-window" title="编辑会员" data-options="modal:true,
	closed:true,resizable:true,iconCls:'icon-save',href:'employee/edit'" style="width:40%;height:70%;padding:10px;">
</div>
<div id="employeeAddWindow" class="easyui-window" title="添加会员" data-options="modal:true,
...
function doSearch_employee(value,name){ //用户输入用户名,点击搜素,触发此函数  
	if(value == null || value == ''){
		...
		$("#employeeList").datagrid({
	        title:'会员列表', singleSelect:false, collapsible:true, pagination:true, rownumbers:true, method:'get',
			nowrap:true, toolbar:"toolbar_employee", url:'employee/list', method:'get', loadMsg:'数据加载中......',
			fitColumns:true,//允许表格自动缩放,以适应父容器
	        columns : [ [ 
				{field : 'ck', checkbox:true },
				{field : 'empId', width : 100, align:'center', title : '会员编号'},
				{field : 'empName', width : 100, align : 'center', title : '会员姓名'},
				{field : 'sex', width : 100, align : 'center', title : '性别', formatter:TAOTAO.formatSex},
				{field : 'idCode', width : 150, title : '身份证号', align:'center'},
				{field : 'education', width : 100, title : 'QQ', align:'center'},
				{field : 'educationForm', width : 100, title : '手机号', align:'center'},
    ...
    function employee_edit(){
    	$.get("employee/edit_judge",'',function(data){
       		if(data.msg != null){
            	...
            	if(ids.length == 0){
            		$.messager.alert('提示','必须选择一个会员才能编辑!');
            		return ;
            	}
            	if(ids.indexOf(',') > 0){
            		$.messager.alert('提示','只能选择一个会员!');
            		return ;
            	}
